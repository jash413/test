//file : components/project/create_edit/stepsAndSchema.ts

import { Step } from '@/components/forms/del-MultiStepForm';
...
export const projectSchema = z.object({
  ...
  address: z.string().min(1, 'Address is required').refine(validateAddress, {
    message:
      "Please enter a valid address (e.g., '123 Main St, City, State ZIP')"
  }),
  start_date: z.coerce.date(),
  end_date: z.coerce.date().optional(),
  budget_estimated: z.number().positive('Budget must be a positive number'),
  actual_spent: z
    .number()
  ...
  project_documents: z
    .any()
    .refine(
      (file): file is File => !file || file instanceof File,
      (file) => ({
        message: `Expected a file object but got ${typeof file}.`
      })
    )
    .refine(
      ...
      (file) => ({
        ...
        }MB. Your file size is ${((file as File).size / 1000000).toFixed(2)}MB.`
      })
    )
    .refine(
...
// You might want to replace this with a more robust address validation library
